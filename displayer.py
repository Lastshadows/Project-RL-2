...
def save_caronthehill_image(position, speed, out_file=None, close=False):
    ...
    if not checked and not os.path.isfile(
            "background_" + str(CANVAS_WIDTH) + "_" + str(
                    CANVAS_HEIGHT) + ".png"):
        ...
        for h in hl:
            ...
            pix += 1
        pygame.image.save(surf, "background_" + str(CANVAS_WIDTH) + "_" + str(
            CANVAS_HEIGHT) + ".png")
        checked = True
    ...
    surf.blit(pt, (round((CANVAS_WIDTH / 2) * (1 + pt_pos1)) - width_pt / 2,
                   CANVAS_HEIGHT - round(((CANVAS_HEIGHT) / 2) * (
                               1 + Hill(pt_pos1))) - height_pt))
    surf.blit(pt, (round((CANVAS_WIDTH / 2) * (1 + pt_pos2)) - width_pt / 2,
                   CANVAS_HEIGHT - round(((CANVAS_HEIGHT) / 2) * (
                               1 + Hill(pt_pos2))) - height_pt))

    # Display the car
    x_car = round((CANVAS_WIDTH / 2) * (1 + position)) - width_car / 2
    h_car = Hill(position)
    h_car_next = Hill(position + step_hill)
    y_car = CANVAS_HEIGHT - round(
        ((CANVAS_HEIGHT) / 2) * (1 + h_car)) - height_car
    angle = ppoints_to_angle((position, position + step_hill),
                             (h_car, h_car_next))
    rot_car, rect = rotate(car,
                           pygame.Rect(x_car, y_car, width_car, height_car),
                           360 - angle)
    ...
    rect = (CANVAS_WIDTH - loc_width_from_bottom - width_speed,
            CANVAS_HEIGHT - loc_height_from_bottom, width_speed,
